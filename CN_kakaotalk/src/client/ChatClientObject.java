...
public class  ChatClientObject extends JFrame implements ActionListener,Runnable
{
    ...
    private JButton fileBtn,sendBtn,exitBtn,inviteBtn;
    ...
    public ChatClientObject() {
        Color back=new Color(155,187,212);//배경 색상

        output = new JTextArea();
        output.setFont(new Font("맑은 고딕",Font.BOLD,15));
        output.setBackground(back);
        ...
        fileBtn = new JButton("파일전송");
        exitBtn= new JButton("나가기");
        inviteBtn=new JButton("초대하기");

        sendBtn.setBackground(Color.gray);
        fileBtn.setBackground(back);
        exitBtn.setBackground(back);
        inviteBtn.setBackground(back);
    ...
    public void service(String uid){
        String serverIP= JOptionPane.showInputDialog(this,"Enter Server  IP: ","192.168.147.1");//메인에서 가져와야함
        ...
        //닉네임 받기
        nickName= uid;
        //JOptionPane.showInputDialog(this,"id를 입력하세요","id" ,JOptionPane.INFORMATION_MESSAGE);//메인에서 가져오기
        ...
        try{
            ...
            System.out.println("Cannot linked Server");
            e.printStackTrace();
            System.exit(0);
        }
        try{
            ...
            dto.setNickName(nickName);
            writer.writeObject(dto);  //역슬러쉬가 필요가s없음
            writer.flush();
        }catch(IOException e){
            e.printStackTrace();
        }
    ...
    String serverIP= JOptionPane.showInputDialog(this,"서버IP를 입력하세요","192.168.147.1");//메인에서 가져와야함
    if(serverIP==null || serverIP.length()==0){  //만약 값이 입력되지 않았을 때 창이 꺼짐
        System.out.println("서버 IP가 입력되지 않았습니다.");
        System.exit(0);
    }
    //닉네임 받기
    nickName= JOptionPane.showInputDialog(this,"id를 입력하세요","id" ,JOptionPane.INFORMATION_MESSAGE);//메인에서 가져오기
    if(nickName == null || nickName.length()==0){
        nickName="guest";
    }
    try{
    ...
    public void actionPerformed(ActionEvent e){
        ...
        try{
            ...
            if(msg.equals("exit")||button.getText().equals("나가기")){
                dto.setCommand(Info.EXIT);
            }
            else if(button.getText().equals("초대하기")){
